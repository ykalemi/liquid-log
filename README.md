...
* В результате должна быть запись `BUILD SUCCESS`, а также должен получиться JAR-файл: `Building jar: ${some_directory}/liquid_log/target/liquid_log-0.0.1-SNAPSHOT.jar`
* Перейти в папку `target`
* Выполнить команду: `java -jar liquid_log-0.0.1-SNAPSHOT.jar`
* `java -DParser -Dparse.mode=sdng -Dinflux.host="http://127.0.0.1:8086" -Dinflux.user="root" -Dinflux.password="root" -jar liquid_log-0.0.1-SNAPSHOT.jar "/home/user/sdng.log.2017-09-07"`
